...
class program_file {
	 ...
	 */
	function __constructor($lang, $sourcecode, $markerid, $timelimit, $sourcepath = "", $extra_path = "", $firstname = "", $lastname = "",   $userid = "",$evaluator=false) {
...
function fetch_tests($testcases, $base_path){
	...
	if($sha1file != $contenthash){
		echo($sha1file." the extracted one-");
		echo($contenthash." the other one");
		unlink($path_zip);
